...
export const fetchAllPromos = createAsyncThunk(
  ...
  async ({ token }, { rejectWithValue }) => {
    try {
      let { data } = await axios.get(VITE_API_URL + `/api/promos`, {
        headers: {
          authorization: token,
        },
      });
...
export const fetchSinglePromo = createAsyncThunk(
  ...
  async ({ id, token }, { rejectWithValue }) => {
    try {
      let { data } = await axios.get(VITE_API_URL + `/api/promos/${id}`, {
        headers: {
          authorization: token,
        },
      });
  ...
  async ({ promoId, updates, token }, { rejectWithValue }) => {
    try {
      const { data } = await axios.put(
        VITE_API_URL + `/api/promos/${promoId}`,
        updates,
        {
          headers: {
            authorization: token,
